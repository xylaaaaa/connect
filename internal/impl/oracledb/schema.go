...
// catalog query, then switches back. This mirrors the Debezium approach and
// avoids both CDB_* view privilege issues and separate-connection login issues.
...
}, table replication.UserTable) (*cachedSchema, error) {
...
func (sc *schemaCache) schemaForEvent(ctx context.Context, table replication.UserTable, eventKeys []string) (any, *columnTypeInfo, error) {
	...
	if sc.pdbName != "" {
		...
		// before querying ALL_TAB_COLUMNS (mirrors the Debezium approach).
