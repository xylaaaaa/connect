...
func VerifyUserTables(ctx context.Context, db DBQuerier, tableFilter *confx.RegexpFilter, log *service.Logger) ([]UserTable, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("fetching user tables for verification: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&ut.Schema, &ut.Name); err != nil {
			return nil, fmt.Errorf("scanning tables row for user tables: %w", err)
		}
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("iterating through tables for user tables: %w", err)
	}
	...
	for i, tbl := range userTables {
		...
		logGroupsErr := db.QueryRowContext(ctx, `SELECT COUNT(*) FROM ALL_LOG_GROUPS WHERE OWNER = :1 AND TABLE_NAME = :2`, tbl.Schema, tbl.Name).Scan(&logGroupsCnt)
		if logGroupsErr != nil {
			return nil, fmt.Errorf("querying log groups for table '%s': %w", tbl.FullName(), logGroupsErr)
		}
		if logGroupsCnt == 0 {
