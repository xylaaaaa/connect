...
func sqlRawOutputConfig() *service.ConfigSpec {
	return service.NewConfigSpec().
		...
		Description(`All messages within a batch are executed in a single database transaction. When consuming from Kafka, messages are automatically ordered by partition within the transaction, allowing `+"`max_in_flight` > 1"+` to parallelize across partitions while preserving consume order within each partition.`).
		Field(driverField).
		Field(dsnField).
		Field(rawQueryField().
...
func (*sqlRawOutput) partitionOrderedIndices(batch service.MessageBatch) []int {
	if len(batch) <= 1 {
		if len(batch) == 1 {
			return []int{0}
		}
